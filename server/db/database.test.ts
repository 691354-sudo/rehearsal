...
describe("database migrations", () => {
  it("upgrades legacy review rows without losing their due date", () => {
    ...
    expect(migrated.prepare("SELECT id FROM schema_migrations ORDER BY id").all()).toHaveLength(11);
    ...
    expect(reopened.prepare("SELECT id FROM schema_migrations ORDER BY id").all()).toHaveLength(11);
