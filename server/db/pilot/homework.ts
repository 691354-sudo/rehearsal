...
export class PilotHomework {
  constructor(private readonly store: PilotStore, private readonly queue: PilotQueue,
    private readonly tutor: TutorRepository, private readonly cores: TutorCores) {}

  active(tutorChatId?: string, language: LanguageCode = "en") {
    const row = this.store.db.prepare(`SELECT homework_id FROM pilot_homework
      WHERE language=? AND status IN ('recall_in_progress', 'tutor_in_progress', 'awaiting_feedback')
        ${tutorChatId ? "AND tutor_chat_id = ?" : ""} ORDER BY started_at DESC LIMIT 1`)
      .get(language,...(tutorChatId ? [tutorChatId] : [])) as { homework_id: string } | undefined;
  ...
  list(language: LanguageCode = "en"): HomeworkSummary[] {
    const rows = this.store.db.prepare(`SELECT h.homework_id, h.tutor_chat_id, h.started_at, h.status, h.plan
      ...
      WHERE t.language_code = ? ORDER BY h.started_at DESC, h.rowid DESC`).all(language) as Array<{
  ...
  create(input: { language?: LanguageCode; homeworkId: string; tutorChatId?: string; requestedMinutes: number; timezone: string },
    ...
    return db.transaction(() => {
      ...
      if (previous) {
        ...
        if (homework.language !== (input.language ?? "en") || homework.requestedMinutes !== input.requestedMinutes
      ...
      const thread = this.tutor.getOrCreateThread(input.tutorChatId, language);
      if (this.active(thread.publicId,language)) throw new PilotError("HOMEWORK_ALREADY_ACTIVE");
      const settings = this.store.settings();
      this.store.identity();
      const total = input.requestedMinutes * 60;
      const ready=this.cores.list(language).slice(0,Math.max(1, Math.floor(total/settings.estimatedTutorSecondsPerCard)));
      if (!ready.length) throw new PilotError("NO_TUTOR_CARDS");
      const plan = {
        timezone:this.store.timezone(input.timezone),requestedMinutes:input.requestedMinutes,
        plannedRecallCards:0,plannedRecallSeconds:0,plannedTutorSeconds:total,
        plannedCardIds:[],plannedTutorRequestIds:[],plannedTutorCardIds:ready.map((entry)=>entry.cardId),
        settingsSnapshot:settings,dueCardsAtStart:this.queue.dueCount(now,language),
      };
      ...
      db.prepare(`INSERT INTO pilot_homework(homework_id, tutor_chat_id, language, started_at,
        status, app_version, experiment_version, plan, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(input.homeworkId, thread.publicId, language, now, "tutor_in_progress",
