...
export type ReviewBatch = {
  publicId: string;
  title: string;
  kind: "chat_review" | "vocab" | "text_import" | "pattern_drill";
...
export function ReviewBatchPanel(props: {
  ...
  return <section className="simple-review-batch">
    ...
    <div className="simple-review-list">
      {visible.map((candidate) => <article className={`simple-review-candidate${selected.has(candidate.id) ? " is-selected" : ""}`} key={candidate.id}>
    ...
    <footer><span>{notice}</span><button className="simple-primary" disabled={!selected.size || saving || props.batch.status === "committed"} onClick={() => void commit()} type="button">
      {saving ? <LoaderCircle className="simple-spin" size={15} /> : <Check size={15} />}{props.batch.status === "committed" ? "Saved" : `Add selected${selected.size ? ` (${selected.size})` : ""}`}</button></footer>
  </section>;
}
