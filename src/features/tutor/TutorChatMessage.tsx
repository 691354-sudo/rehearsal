...
export function TutorChatMessage({ learnerMessage, message, onDelete, onEdit, onRetry }: {
  ...
  return <article aria-label={message.role === "assistant" ? "Tutor response" : "Your message"} className={`simple-message simple-message--${message.role}`} data-status={message.status}>
