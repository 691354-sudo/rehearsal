...
export function usePracticeTopics(language: Language, value: string, onTopic: (value: string) => void, revision = 0) {
  ...
  useEffect(() => {
    const controller = new AbortController(); setError(""); setLoading(true); setCollectionItems([]);
    void (async () => {
      ...
      if (set.kind !== "all") {
        const collection = set.kind === "category" ? await getCategory(set.id, controller.signal)
          : await getTopic(set.kind === "liked" ? "liked" : set.id, controller.signal,language);
        if (!controller.signal.aborted) setCollectionItems(collection.items);
      }
  ...
  return { topics, collectionItems, selectedTopicItems: value ? collectionItems.map((item) => item.publicId) : null,
    error, loading, retry: () => setRetryCount((current) => current + 1) };
}
