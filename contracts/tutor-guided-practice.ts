...
export const isGuidedPracticeStartMessage = (content: string) => guidedPracticeMessages.has(content.trim()) || isRecallPracticeStartMessage(content);
...
export const guidedPracticeReviewMessages = (messages: ConversationMessage[]) => {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    if (messages[index].role !== "user" || !isGuidedPracticeStartMessage(messages[index].content)) continue;
