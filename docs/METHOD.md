...
The core production cycle is `Capture → Review → Library → Listen & Repeat → Recall → Learned`: capture real Russian thoughts, approve natural target-language cards, hear and shadow selected cards while walking, then retrieve the due cards from memory in writing. For a language with audio, Practice opens in Listen & Repeat; a written-only language opens in Recall. FSRS controls future due dates. English uses the server-driven pilot below; other languages retain the existing client session queue.
...
The `echo-learning-pilot-v1` flow applies only to English. Interface labels stay English; Homework conversation and explanations are Russian, with English target phrases. Notebook, other languages, fonts and the Echo visual system keep their existing behavior.

- A new card becomes Recall-eligible after five distinct Listen appearances with at least one complete audio playback. Repetitions and manual Replay within an appearance count once; a new automatic appearance or a new session can count again. Partial playback and early Stop do not count. Appearance IDs, pending events and the paused queue survive reload. Normal progress sync stays silent; failed sync offers Retry in the existing progress row. Existing reliable Recall history remains eligible; historical Listen counts are not fabricated.
- Active Recall always offers a text field, but an oral attempt is equally valid. A typed answer uses Check answer: exact normalized reference/accepted matches are checked locally, and other wording is checked semantically by AI for meaning and English grammar. A normalized match to the card's reference is green (Correct), without a comment. Valid different wording, including saved accepted alternatives, is amber (Valid alternative) with the fixed note that the card uses different wording. Only errors are red (Needs a correction), with a brief generated explanation, correction and highlighted typed fragments. Each state has distinct text and an icon. Checking, unavailable checks and oral self-checks are neutral. The card answer remains visible if checking fails, with an explicit Retry check. Checking never chooses the memory grade. An oral attempt uses Show answer. Both paths precede an explicit Again/Hard/Good/Easy grade. Good is the initial selection on every reveal; it is never saved automatically. Grades stay in one horizontal row. Similarity does not pick a grade. Hard/Good/Easy each add one cumulative successful recall; Again does not. Only a graded Recall changes FSRS, using uniform retention 0.90 and the same neutral scheduling preset regardless of Like.
- The server selects from the whole profile DB: due Recall history first by due, then unseen cards meeting the Listen threshold by eligibility timestamp, with card ID ties. The daily new-card limit counts distinct first recalls across every session in the saved local timezone. A future due date is never pulled forward to fill a session. Learned cards must return to learning before joining English Recall; a Library review link does not bypass admission.
- Like keeps playback running, adds the card to the derived **Liked** Topic and queues one pending Tutor request. Liked is always available in Topic browsing and Practice, including old likes. It does not own, copy, rename, merge or delete cards; their original Topic remains intact. Old likes do not bulk-create requests. Unlike removes the collection membership and cancels an unstarted request. A request resolves only after a saved card-specific Tutor activity and a linked learner reply; unfinished requests return to the next Homework.
- Homework begins from Tutor with a requested whole number of minutes and opens a separate chat for each new session. For `T` seconds, reserve `L = min(pending, max(1, floor(T/U)))` liked cards, `likeSeconds = min(T, L*U)`, then `N = min(eligible unselected cards, floor((T-likeSeconds)/(R+U)))`. Defaults are `R=20`, `U=60`; the immutable plan reserves `N*R` for Recall and `likeSeconds+N*U` for Tutor. Requests are assigned exclusively. Tutor-only and shorter plans are valid. Homework entries use `DD.MM.YYYY - Homework` in the timezone frozen at creation. A pinned, collapsible Homework folder above Today/Earlier lists every saved Homework whose chat still exists, including legacy Homework records that shared a chat; selecting a Homework shows only messages tagged with that session. Review cards uses only that Homework, including when opened from history. Regular chats remain outside the folder.
- Homework Recall starts directly, returns to the same chat where Tutor automatically sends its first reply after history loads (recovering an interrupted start with the same idempotency key), and permits at most two accepted attempts per card, only after the next FSRS due. Exhausting the queue or Recall reserve offers Return to Tutor; an in-flight attempt can finish. Actual visible active time includes audio and Tutor waiting, excludes hidden/idle time after 120 seconds and feedback, and is explicitly null when measurement is lost. Exhausting total time, or losing its measurement, requires the learner to choose Continue before new tasks.
- Tutor receives only the selected likes and saved Recall results. It prioritizes likes, any Again, then any Hard. Explain unfamiliar phrases; context exercises require two cumulative successful recalls, except a selected like can be practised immediately. Semantic sections distinguish the learner phrase, correction/alternative, explanation and next turn only when relevant; plain conversation stays plain. Every active Homework Tutor reply ends with one concrete next action; after a wording or grammar question it resumes the unfinished exercise, or directs the learner to End session when time is up. Tutor never assigns Recall scores or changes FSRS.
- End session enters `awaiting_feedback` and reveals the feedback form after the last chat message within the existing message scroll area. Difficulty, time fit and next-step clarity are required; helpfulness and two free-text responses are optional. Finish Homework saves feedback and completion atomically. Continue later retains the draft. Cancel preserves accepted reviews and releases pending requests; deleting the chat also cancels its Homework.

Observation enrollment is separate from feature availability. Each confirmed participant gets a real start snapshot and an actual closing snapshot for their seven-day window. Eight datasets and reproducible metrics preserve missing boundaries, partial time, card imports/deletions/edits and zero denominators (`null`). Audio, full chats and typed answers are excluded from the pilot export. Enrollment/export operations are documented in [OPERATIONS.md](OPERATIONS.md#english-learning-pilot).
...
An empty Tutor chat keeps `Start for me` as one separate primary action and shows four exercise cards with short outcome descriptions: `Tell it better`, `Recall & reuse`, `Role-play twice`, and `Read → retell`. The primary action chooses a short guided practice from current due material when useful and otherwise begins with the learner's own thought. Selecting a card starts that recipe directly; `Read → retell` asks for a pasted or uploaded text when none is present. Guided practice gives one action at a time, keeps one topic for no more than three training rounds, asks for self-repair before revealing a reformulation, and ends with whole-thought reproduction plus use in a new context. Voice and typing remain equivalent. Tutor does not score pronunciation, enforce timers, grade FSRS, or create a second schedule.
