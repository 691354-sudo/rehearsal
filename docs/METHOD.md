...
Practice selection lists show compact category labels instead of a learning-status badge. Cards without a category show their owning Topic. Names remain visible and wrap when needed; there are no extra disclosure controls. Recall/listen counts remain visible.
...
- Started cards are ordered by oldest full playback, then card ID. In all Library, new cards first match categories of started LR cards, then their Topics, then the rest; oldest creation time wins within each tier. With no started cards, use creation time. Membership in several categories never duplicates a card.
...
- **To Recall** is available in the player and the LR list's card menu. It moves an LR card immediately, preserves its counts and FSRS, and does not stop or advance playback. Returning an already admitted card does not restart it. Cards beyond LR remain playable through ordinary Library/Category/Topic listening but leave LR recommendations.
...
- **Active Recall recommendations:** 10 or 20 unique cards. Fill with due FSRS cards, then arrivals from LR/To Recall. Learning/Relearning come first; Review sorts by lowest retrievability, then oldest due date and card ID. Arrivals sort by entry time. If places remain, borrow initial LR cards at exactly 4/5, at most 20% of the requested size (2 or 4). Returned cards cannot be borrowed. Leave any remaining places empty. Future FSRS reviews are not pulled forward automatically.
