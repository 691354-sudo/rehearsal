...
- Active Recall always offers a text field, but an oral attempt is equally valid. A typed answer uses Check answer: exact normalized reference/accepted matches are checked locally, and other wording is checked semantically by AI for meaning and English grammar. Valid alternatives are accepted. Correct and Needs a correction have separate text, icons and green/red feedback; errors highlight the relevant typed fragments. Checking, unavailable checks and oral self-checks are neutral. The card answer remains visible if checking fails, with an explicit Retry check. Checking never chooses the memory grade. An oral attempt uses Show answer. Both paths precede an explicit Again/Hard/Good/Easy grade. Good is the initial selection on every reveal; it is never saved automatically. Grades stay in one horizontal row. Similarity does not pick a grade. Hard/Good/Easy each add one cumulative successful recall; Again does not. Only a graded Recall changes FSRS, using uniform retention 0.90 and the same neutral scheduling preset regardless of Like.
