...
- Homework begins in the current Tutor chat with a requested whole number of minutes. For `T` seconds, reserve `L = min(pending, max(1, floor(T/U)))` liked cards, `likeSeconds = min(T, L*U)`, then `N = min(eligible unselected cards, floor((T-likeSeconds)/(R+U)))`. Defaults are `R=20`, `U=60`; the immutable plan reserves `N*R` for Recall and `likeSeconds+N*U` for Tutor. Requests are assigned exclusively. Tutor-only and shorter plans are valid. Homework entries use `DD.MM.YYYY - Homework` in the timezone frozen at creation. A pinned, collapsible Homework folder above Today/Earlier lists every saved Homework whose chat still exists, including multiple Homework records in one chat; regular chats remain outside it.
...
- Tutor receives only the selected likes and saved Recall results. It prioritizes likes, any Again, then any Hard. Explain unfamiliar phrases; context exercises require two cumulative successful recalls, except a selected like can be practised immediately. Semantic sections distinguish the learner phrase, correction/alternative, explanation and next turn only when relevant; plain conversation stays plain. Tutor never assigns Recall scores or changes FSRS.
