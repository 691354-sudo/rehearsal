...
An empty Tutor chat offers `Start for me` and `Choose an exercise`. The first action chooses a short guided practice from current due material when useful and otherwise begins with the learner's own thought; the second offers only `Tell it better`, `Recall & reuse`, and `Role-play twice`. A supplied substantial text may instead enter `Read → retell`. Guided practice gives one action at a time, keeps one topic for no more than three training rounds, asks for self-repair before revealing a reformulation, and ends with whole-thought reproduction plus use in a new context. Voice and typing remain equivalent. Tutor does not score pronunciation, enforce timers, grade FSRS, or create a second schedule.
