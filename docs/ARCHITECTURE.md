...
Workload roles are pinned: Sol for Tutor conversation, Terra for material generation and review, and Luna for small utility tasks. Model changes are manual and may be canary-checked with `npm run models:check`; runtime configuration is never rewritten automatically. Roman receives the existing personal context, while Oliver and Zanna receive neutral Russian-speaking-adult personas and an instruction not to invent personal facts. Source text, Tutor history, individual messages, and model output are bounded before provider calls. Tutor considers up to 200 stored messages before applying its 40,000-character history ceiling; this avoids shifting an otherwise short conversation window after every turn. Requests use a stable key derived from the profile-local thread UUID so the growing exact prefix can reuse OpenAI prompt caching without joining different conversations. Completed assistant-message metadata and structured server logs store only aggregate request, input, cache-write, cached-input, output, reasoning, total-token, and context-size counts; they do not log conversation text.
